...
export class DatabaseService {
  ...
  constructor() {
    this.db = {
      commands: new PouchDB(environment.database.commands.url, {auth: {
        username: environment.database.username,
        password: environment.database.password
      }}),
      keywords: new PouchDB(environment.database.keywords.url, {auth: {
        username: environment.database.username,
        password: environment.database.password
      }}),
      documentation: new PouchDB(environment.database.documentation.url, {auth: {
        username: environment.database.username,
        password: environment.database.password
      }}),
  ...
  async get(dbName: string, id: string): Promise<any> {
    ...
    return this.db[dbName].get(id).then(({_id, _rev, ...doc}: any) => doc).catch((err: Error) => {
      console.error(err);
      return null;
    });
  }
  ...
  async put(dbName: string, doc: any): Promise<any> {
    ...
    return this.db[dbName].put(doc).then((response: any) => {
